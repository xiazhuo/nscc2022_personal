...
//若路径存在则会初始化对应的RAM,否则不初始化，一般仅需初始化baseram(即指令存储器)
parameter BASE_RAM_INIT_FILE = "thinpad_top.srcs\\sim_1\\testbin\\self_test\\test.bin"; //BaseRAM初始化文件，请修改为实际的绝对路径
parameter EXT_RAM_INIT_FILE = "thinpad_top.srcs\\sim_1\\testbin\\self_test\\data.bin";    //ExtRAM初始化文件，请修改为实际的绝对路径
